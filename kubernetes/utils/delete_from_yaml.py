...
def delete_from_directory(
    ...
    """
    Perform a delete action from files from a directory. Pass True for verbose to
    ...
    verbose: If True, print confirmation from the delete action.
        Default is False.
    namespace: string. Contains the namespace to delete all
        resources inside. The namespace must preexist otherwise
        the resource deletion will fail. If the API object in
        the yaml file already contains a namespace definition
        this parameter has no effect.
    apply: bool. If True, use server-side apply for deleting resources.

    Available parameters for deleting <kind>:
    ...
    Returns:
        The list containing the deleted kubernetes API objects.
...
def delete_from_yaml(
    ...
    **kwargs,
    ...
    """
    Perform a delete action from a yaml file. Pass True for verbose to
    print confirmation information.
    Input:
    ...
    def delete_with(objects, apply=apply):
        failures = []
        k8s_objects = []
    ...
    if yaml_objects:
        ...
        with open(os.path.abspath(yaml_file)) as f:
            ...
            return delete_with(yml_document_all, apply)
    else:
        raise ValueError(
...
def delete_from_dict(
    ...
    """
    Perform a delete action from a dictionary containing valid kubernetes
...
def delete_from_yaml_single_item(
    ...
    if hasattr(k8s_api, "delete_namespaced_{0}".format(kind)):
        # Decide which namespace we are going to delete the object from,
        # if any
